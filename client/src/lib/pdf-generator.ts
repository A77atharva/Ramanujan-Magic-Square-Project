...
function header(doc: jsPDF, W: number, logoSrc: string|null) {
  ...
  if (logoSrc) { try { doc.addImage(logoSrc,'PNG', W/2-8, 1, 16, 20); } catch {
    try { doc.addImage(logoSrc,'JPEG', W/2-8, 1, 16, 20); } catch {}
  }}
}
...
export const generateMagicSquarePDF = async (data: MagicSquareData, org?: OrgContext | null) => {
  ...
  doc.text("Each block consists of four adjacent/consecutive cells (including wrap-around edges)", W/2, 45, {align:"center"});
  const cs5 = 10, cols3 = 3;
  const panW5 = (W - mg*2 - (cols3-1)*6) / cols3, panH5 = cs5*4 + 12, startY5 = 51;
  ...
  sy += 14;
  st(doc, C.textLight); doc.setFont("helvetica","italic"); doc.setFontSize(11);
